<x-layout title="Add Organization Program">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <h2 class="text-xl font-bold mb-4">Add Program for {{ $studentOrganization->nama }}</h2>
            <form action="{{ route('student-organizations.organization-programs.store', $studentOrganization->id) }}" method="post" id="form-program">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
                <div class="col-span-1 md:col-span-2">
                    <div class="flex flex-col gap-1 mb-5">
                        <label for="nama_program">Nama Program</label>
                        <input type="text" name="nama_program" value="{{ old('nama_program') }}" class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="kategori" id="kategori" class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-select border-gray-300 border-2 p-2 rounded-md">
                            ...
                            <option value="pengembangan_organisasi" {{ old('kategori') == 'pengembangan_organisasi' ? 'selected' : '' }}>Pengembangan Organisasi</option>
                            <option value="kaderisasi" {{ old('kategori') == 'kaderisasi' ? 'selected' : '' }}>Kaderisasi</option>
                            <option value="keilmuan_keprofesian" {{ old('kategori') == 'keilmuan_keprofesian' ? 'selected' : '' }}>Keilmuan Keprofesian</option>
                            <option value="pengabdian_masyarakat" {{ old('kategori') == 'pengabdian_masyarakat' ? 'selected' : '' }}>Pengabdian Masyarakat</option>
                            <option value="minat_bakat" {{ old('kategori') == 'minat_bakat' ? 'selected' : '' }}>Minat Bakat</option>
                            <option value="kesejahteraan_mahasiswa" {{ old('kategori') == 'kesejahteraan_mahasiswa' ? 'selected' : '' }}>Kesejahteraan Mahasiswa</option>
                            <option value="kewirausahaan" {{ old('kategori') == 'kewirausahaan' ? 'selected' : '' }}>Kewirausahaan</option>
                            <option value="komunikasi_informasi" {{ old('kategori') == 'komunikasi_informasi' ? 'selected' : '' }}>Komunikasi Informasi</option>
                        </select>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="date" name="tanggal_pelaksanaan" value="{{ old('tanggal_pelaksanaan') }}" class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
