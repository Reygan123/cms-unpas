...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row justify-content-between">
                            <div class="col-sm-6">
                                <a href="{{ route('admin.alumni.create') }}" class="btn btn-primary btn-rounded">
                                    <span class="btn-icon-left text-primary"><i
                                            class="fa-solid fa-pen-to-square"></i></span>Add alumni
                                </a>
                            </div>
                            <div class="col-sm-6">
                                <form action="{{ route('admin.alumni.index') }}" method="GET" class="d-flex">
                            ...
                            <table class="table table-responsive-sm">
                                ...
                                <tbody>
                                    @forelse($alumnis as $alumni)
                                        <tr>
                                            <th>
                                                <input type="checkbox" name="alumnis[]" value="{{ $alumni->id }}">
                                            </th>
                                            <td>
                                                <h6>{{ $alumni->title }}</h6>
                                                <div>{{ date('d M Y', strtotime($alumni->pub_date)) }} |
                                                    {{ $alumni->category->name }}</div>
                                                <div class="flex mt-4">
                                            ...
                                            <td>
                                                @if ($alumni->image && file_exists(public_path('storage/alumnis/' . $alumni->image)))
                                                    <img src="{{ asset('storage/alumnis/' . $alumni->image) }}"
                                                        alt="{{ $alumni->title }}" class="admin-index-image">
                                                @else
    ...
    <script>
        ...
        document.getElementById('select-all').onclick = function() {
            var checkboxes = document.getElementsByName('alumnis[]');
        ...
        document.getElementById('delete-selected').onclick = function() {
            ...
            for (var checkbox of checkboxes) {
                if (checkbox.checked) {
                    selectedalumnis.push(checkbox.value);
                }
            }
