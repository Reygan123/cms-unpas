@extends('layouts.app', ['title' => 'Activity'])
                ...
                <div class="card border-0 shadow">
                    <div class="card-header">
                        <h6 class="m-0 font-weight-bold">
                            <i class="fas fa-question-circle"></i> ACTIVITY
                        </h6>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="row mb-4">
                            <div class="col-md-8">
                                <form action="{{ route('admin.activity.index') }}" method="GET" id="filterForm"
                                    class="form-inline">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @forelse($activity as $no => $item)
                                        <tr>
                                            <td>
                                                <input type="checkbox" class="checkbox" value="{{ $item->id }}">
                                            </td>
                                            <th scope="row">
                                                {{ ++$no + ($activity->currentPage() - 1) * $activity->perPage() }}
                                            </th>
                                            <td>{{ $item->service->name }}</td>
                                            <td>{{ $item->title }}</td>
                                            <td>
                                                @if ($item->image)
                                                    <img src="{{ asset('storage/activities/' . $item->image) }}"
                                                        width="100">
                                                @endif
                                            </td>
        ...
        $('#delete-selected').click(function() {
            ...
            if (selectedIds.length > 0) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.activity.massDestroy') }}",
                            type: "DELETE",
                            data: {
                                "ids": selectedIds,
                                "_token": token,
