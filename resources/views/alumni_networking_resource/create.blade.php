<x-layout title="Add Alumni Networking Resource">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('alumni-networking-resources.store') }}" method="post" enctype="multipart/form-data" id="form-alumni-networking">
            @csrf
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:o utline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('judul')
                            <div>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <option value="video_sharing" {{ old('tipe') == 'video_sharing' ? 'selected' : '' }}>Video Sharing</option>
                        <option value="komunitas_profesi" {{ old('tipe') == 'komunitas_profesi' ? 'selected' : '' }}>Komunitas Profesi</option>
                        </select>
                        @error('tipe')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
