<x-layout title="Add Alumni">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('departement.alumni.store', $departement->id) }}" method="post" enctype="multipart/form-data" id="form-alumni">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="cerita_sukses" id="cerita_sukses" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ old('cerita_sukses') }}</textarea>
                        @error('cerita_sukses')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
