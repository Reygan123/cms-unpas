<x-layout title="Edit Alumni">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('departement.alumni.update', [$departement->id, $alumnus->id]) }}" method="post" enctype="multipart/form-data" id="form-alumni">
            @csrf
            @method('PUT')
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="text" name="profesi" value="{{ old('profesi', $alumnus->profesi) }}"
                                class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                            @error('profesi')
                                <div>
                                    <small class="text-red-500"><i>{{ $message }}</i></small>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="text" name="perusahaan" value="{{ old('perusahaan', $alumnus->perusahaan) }}"
                                class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                            @error('perusahaan')
                                <div>
                                    <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        <label for="cerita_sukses">Cerita Sukses</label>
                        <textarea name="cerita_sukses" id="cerita_sukses" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ old('cerita_sukses', $alumnus->cerita_sukses) }}</textarea>
                        @error('cerita_sukses')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                                id="image-preview" 
                                src="{{ old('foto') ? asset('storage/alumni-photo/' . old('foto')) : ($alumnus->foto ? asset('storage/' . $alumnus->foto) : '#') }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ $alumnus->foto ? '' : 'hidden' }} z-0 rounded-md" 
                            />
                            <button
                                id="remove-btn"
                                type="button"
                            ...
                            <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ $alumnus->foto ? 'hidden' : '' }}">
            ...
            <div class="flex items-center justify-between my-5">
                <a href="{{ route('departement.alumni.index', $departement->id) }}" class="py-3 px-5 bg-gray-300 rounded-xl">Back</a>
