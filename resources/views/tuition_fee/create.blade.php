<x-layout title="Add Tuition Fee">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('tuition-fees.store') }}" method="post" id="form-tuition-fee">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <select name="jenis_program" id="jenis_program"
                        ...
                        <option value="reguler" {{ old('jenis_program') == 'reguler' ? 'selected' : '' }}>Reguler</option>
                        <option value="hybrid" {{ old('jenis_program') == 'hybrid' ? 'selected' : '' }}>Hybrid</option>
                        <option value="pjj" {{ old('jenis_program') == 'pjj' ? 'selected' : '' }}>PJJ</option>
                        <option value="fast_track" {{ old('jenis_program') == 'fast_track' ? 'selected' : '' }}>Fast Track</option>
                    </select>
                    @error('jenis_program')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <input type="text" name="jenis_biaya" value="{{ old('jenis_biaya') }}"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    @error('jenis_biaya')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="flex flex-col gap-1 mb-5 md:col-span-2">
                    ...
                    <input type="number" name="nominal" value="{{ old('nominal') }}" min="0" step="any"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    @error('nominal')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="flex flex-col gap-1 mb-5 md:col-span-2">
                    ...
                    <textarea name="keterangan" id="keterangan" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md" rows="3">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                        </div>
