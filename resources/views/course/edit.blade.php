<x-layout title="Edit Course">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('course.update', $course->id) }}" method="post" enctype="multipart/form-data" id="form-course">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
                <div class="flex flex-col gap-1 mb-5">
                    <label for="kode_mata_kuliah">Kode Mata Kuliah</label>
                    <input type="text" name="kode_mata_kuliah" value="{{ old('kode_mata_kuliah', $course->kode_mata_kuliah) }}"
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <input type="text" name="nama_mata_kuliah" value="{{ old('nama_mata_kuliah', $course->nama_mata_kuliah) }}"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <input type="number" name="sks" value="{{ old('sks', $course->sks) }}" min="1" max="10" step="1"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    @error('sks')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <input type="number" name="semester" value="{{ old('semester', $course->semester) }}" min="1" max="14" step="1"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    @error('semester')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <input type="text" name="program_studi" value="{{ old('program_studi', $course->program_studi) }}"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    @error('program_studi')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <input type="text" name="dosen_pengampu" value="{{ old('dosen_pengampu', $course->dosen_pengampu) }}"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                ...
                <div class="flex flex-col gap-1 mb-5 col-span-1 md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md h-24">{{ old('deskripsi', $course->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div>
                            <small class="text-red-500"><i>{{ $message }}</i></small>
                        </div>
                ...
                <div class="flex flex-col gap-1 mb-5 col-span-1 md:col-span-2">
                    <label for="silabus">Silabus (PDF, Max : 2MB) - Biarkan kosong jika tidak ingin mengubah</label>
                    <input type="file" name="silabus" id="silabus" accept=".pdf"
                        class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                    @if(isset($course) && $course->silabus)
                        <div class="mt-2 text-xs">
                            <a href="{{ asset('storage/' . $course->silabus) }}" target="_blank" class="text-blue-500 hover:underline">Lihat Silabus Saat Ini</a>
                        </div>
                    @endif
                    @error('silabus')
