<x-layout title="Class Representatives">
    <div class="p-5">
        <div class="flex items-center justify-between">
            <a href="{{ route('class-representatives.create') }}" class="py-2 px-3 bg-[#5676ff] rounded-full flex items-center hover:bg-gray-900 text-white gap-3 text-xs">
                ...
                <span>Add Class Representative</span>
            </a>
            <form action="{{ route('class-representatives.index') }}" method="GET" class="mb-4 flex items-center gap-2 mt-4">
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y">
                        @foreach ($classRepresentatives as $cr)
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3 text-sm">
                                {{ str_replace('_', ' ', ucwords($cr->jabatan, '_')) ?? '' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $cr->departement->name ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                @if($cr->status_on_duty)
                                    <span class="px-2 py-1 leading-tight text-green-700 bg-green-100 rounded-full">On Duty</span>
                                @else
                                    <span class="px-2 py-1 leading-tight text-gray-700 bg-gray-100 rounded-full">Off Duty</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a
                                        href="{{ route('class-representatives.edit', $cr->id) }}"
                                    ...
                                    <form action="{{ route('class-representatives.destroy', $cr->id) }}" method="POST" class="delete-form" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            type="button"
