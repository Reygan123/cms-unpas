<x-layout title="Edit Class Representative">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('class-representatives.update', $classRepresentative->id) }}" method="post" enctype="multipart/form-data" id="form-class-representative">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                <div class="col-span-1 md:col-span-2 gap-5">
                    <div class="flex flex-col gap-1 mb-5 md:col-span-2">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="status_on_duty" id="status_on_duty" value="1"
                                {{ old('status_on_duty', $classRepresentative->status_on_duty) ? 'checked' : '' }}
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="angkatan" value="{{ old('angkatan', $classRepresentative->angkatan) }}"
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $classRepresentative->nama) }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('nama')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="jabatan" id="jabatan"
                            ...
                            <option value="ketua_angkatan" {{ old('jabatan', $classRepresentative->jabatan) == 'ketua_angkatan' ? 'selected' : '' }}>Ketua Angkatan</option>
                            <option value="pic_aktivis" {{ old('jabatan', $classRepresentative->jabatan) == 'pic_aktivis' ? 'selected' : '' }}>PIC Aktivis</option>
                        </select>
                        @error('jabatan')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="id_departement" id="id_departement"
                            ...
                            @foreach ($departements ?? [] as $departement)
                                <option value="{{ $departement->id }}" {{ old('id_departement', $classRepresentative->id_departement) == $departement->id ? 'selected' : '' }}>{{ $departement->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="kontak" value="{{ old('kontak', $classRepresentative->kontak) }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('kontak')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="deskripsi" id="deskripsi" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ old('deskripsi', $classRepresentative->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                                id="image-preview" 
                                src="{{ old('foto') ? asset('storage/class-representative-photo/' . old('foto')) : (isset($classRepresentative) && $classRepresentative->foto ? asset('storage/' . $classRepresentative->foto) : '#') }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ (isset($classRepresentative) && $classRepresentative->foto) ? '' : 'hidden' }} z-0 rounded-md" 
                            />
                            <button
                                id="remove-btn"
                                type="button"
                            ...
                            <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ (isset($classRepresentative) && $classRepresentative->foto) ? 'hidden' : '' }}">
