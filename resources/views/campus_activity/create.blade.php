<x-layout title="Add Campus Activity">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('campus-activities.store') }}" method="post" enctype="multipart/form-data" id="form-campus-activity">
            @csrf
            @if ($errors->any())
                <div class="mb-5 p-4 bg-red-100 border border-red-400 text-red-700 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <option value="kegiatan_kemahasiswaan" {{ old('kategori') == 'kegiatan_kemahasiswaan' ? 'selected' : '' }}>Kegiatan Kemahasiswaan</option>
                        <!-- <option value="pengabdian_masyarakat" {{ old('kategori') == 'pengabdian_masyarakat' ? 'selected' : '' }}>Pengabdian Masyarakat</option> -->
                        <option value="lomba_kompetisi" {{ old('kategori') == 'lomba_kompetisi' ? 'selected' : '' }}>Lomba/Kompetisi</option>
                        <option value="seminar_workshop" {{ old('kategori') == 'seminar_workshop' ? 'selected' : '' }}>Seminar/Workshop</option>
                        ...
                        <option value="suasana_kampus" {{ old('kategori') == 'suasana_kampus' ? 'selected' : '' }}>Suasana Kampus</option>
                        </select>
                        @error('kategori')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="ringkasan" id="ringkasan" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ old('ringkasan') }}</textarea>
                        @error('ringkasan')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="konten" id="konten" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ old('konten') }}</textarea>
                        @error('konten')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="url" name="tautan_pendaftaran" value="{{ old('tautan_pendaftaran') }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        <label for="gambar">Poster (Max : 2MB)</label>
