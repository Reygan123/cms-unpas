<x-layout title="Add Career Event">
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('career-events.store') }}" method="post" enctype="multipart/form-data" id="form-career-event">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <option value="seminar_karier" {{ old('jenis') == 'seminar_karier' ? 'selected' : '' }}>Seminar Karier</option>
                        <option value="pelatihan_wawancara" {{ old('jenis') == 'pelatihan_wawancara' ? 'selected' : '' }}>Pelatihan Wawancara</option>
                        <option value="personal_branding" {{ old('jenis') == 'personal_branding' ? 'selected' : '' }}>Personal Branding</option>
                        <option value="simulasi_rekrutmen" {{ old('jenis') == 'simulasi_rekrutmen' ? 'selected' : '' }}>Simulasi Rekrutmen</option>
                        <option value="pelatihan_soft_skills" {{ old('jenis') == 'pelatihan_soft_skills' ? 'selected' : '' }}>Pelatihan Soft Skills</option>
                        <option value="pengenalan_profesi" {{ old('jenis') == 'pengenalan_profesi' ? 'selected' : '' }}>Pengenalan Profesi</option>
                        </select>
                        @error('jenis')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
