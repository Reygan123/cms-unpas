<x-layout title="Accreditations">
    <div class="p-5">
        <div class="flex items-center justify-between">
            <a href="{{ route('accreditations.create') }}" class="py-2 px-3 bg-[#5676ff] rounded-full flex items-center hover:bg-gray-900 text-white gap-3 text-xs">
            ...
            <form action="{{ route('accreditations.index') }}" method="GET" class="mb-4 flex items-center gap-2 mt-4">
                    ...
                    <tbody class="bg-white divide-y">
                        ...
                        <tr class="text-gray-700">
                            <td class="px-4 py-3 text-sm">
                                {{ $accreditation->departement ? $accreditation->departement->name : 'Tingkat Fakultas' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $accreditation->jenjang ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $accreditation->lembaga ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $accreditation->status ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $accreditation->nomor_sk ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $accreditation->tanggal_berlaku ? $accreditation->tanggal_berlaku->format('d/m/Y') : '-' }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('accreditations.edit', $accreditation->id) }}"
                                    ...
                                    <form action="{{ route('accreditations.destroy', $accreditation->id) }}" method="POST" class="delete-form" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
