...
class APIController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Post::with(['category', 'user:id,name,email'])
            ->whereDate('pub_date', '<=', $today) // Tambahkan filter ini
    ...
    public function getHowServiceByServiceSlug($service_slug)
    {
        ...
        $howServices = HowService::with('service')
            ->where('service_id', $service->id)
            ->latest()
            ->paginate(10);
    ...
    public function getBonusServiceAll(Request $request)
    {
        ...
        // Transform the response to include full image URLs
        $bonusServices->getCollection()->transform(function ($bonusService) {
            return $this->transformBonusService($bonusService);
        });
